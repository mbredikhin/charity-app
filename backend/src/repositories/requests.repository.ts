...
const findRequestQueryBase = `
  SELECT r.id, r.title, r.description, r.goal_description AS "goalDescription", r.ending_date AS "endingDate",
  (
  ...
  (
    SELECT jsonb_build_object('title', title, 'isVerified', is_verified)
    FROM organizations o
    INNER JOIN request_organization ro ON ro.request_id = r.id AND ro.organization_id = o.id 
  ) AS organization,
  (
    SELECT array_agg(jsonb_build_object('stepLabel', step_label, 'isDone', is_done)) 
